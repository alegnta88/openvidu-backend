...
const OPENVIDU_URL = process.env.OPENVIDU_URL || "http://localhost:7881";
...
router.post('/create-session', authRole(['agent']), async (req, res) => {
    const { phone, customSessionId } = req.body;
    ...
    try {
        // Attempt to create a new OpenVidu session
        ...
        if (err.response?.status === 409) {
            ...
            sessionId = customSessionId || "default-session";
        } else {
            console.error("OpenVidu session creation error:", err.response?.data || err.message);
    ...
    try {
        ...
        console.error("OpenVidu token generation error:", err.response?.data || err.message);
